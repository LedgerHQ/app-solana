...
//
// An INSTRUCTION_INFO descriptor (PROVIDE INSTRUCTION INFO, 0x24) commits to the
// set of substructure TLVs that will follow it (PROVIDE INSTRUCTION SUBSTRUCTURE,
// 0x25) by carrying their SHA-256 as a single target digest. This module owns the
// running SHA-256 over the received substructure TLVs and reports when that
// running digest reaches the committed target.
//
// Templates are streamed strictly sequentially: a 0x24 opens a template, its
// 0x25 substructures fill it, then the next 0x24 opens the next. Only one
...
//
// Lifecycle:
//   cs_substructure_begin()  at each 0x24, after the target digest is extracted
//   cs_substructure_update() at each 0x25, over the substructure TLV bytes
//   cs_substructure_check_complete() polled after each update to latch the template
