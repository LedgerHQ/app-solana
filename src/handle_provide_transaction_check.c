...
static int handle_provide_transaction_check_internal(void) {
    ...
    // 2. Validate chain_id if received: 1=mainnet-beta, 2=devnet, 3=testnet
    if (tlv_output.chain_id_received) {
        PRINTF("[TX CHECK] chain_id=%d\n", (int) tlv_output.chain_id);
        if (tlv_output.chain_id < 1 || tlv_output.chain_id > 3) {
...
static bool check_transaction_check_report_validity(void) {
    ...
    // 1. Check type is TRANSACTION (already enforced at reception, but be defensive)
    if (G_transaction_check_info.type != TRANSACTION_CHECK_TYPE_TRANSACTION) {
        PRINTF("[TX CHECK] Type mismatch: %d\n", G_transaction_check_info.type);
        return false;
    }

    // 3. Check tx_hash: SHA-256 of the raw transaction message
    ...
    // 4. Check address: derive pubkey from derivation path and compare base58
