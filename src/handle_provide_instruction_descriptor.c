...
int validate_instruction_using_descriptor(const MessageHeader *header,
    ...
    if (instruction_descriptor->amount_size == 0) {
        ...
        if (instruction_descriptor->capped_amount_bps_received) {
            ...
            // Main asset instruction: the on-chain spend must not exceed the advertised swap
            // amount. It may be less, the remainder being consumed by other (descriptor-bound)
            // instructions such as a partner fee
