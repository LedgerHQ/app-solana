...
static bool handle_not_valid_after(const tlv_data_t *data, tlv_extracted_t *tlv_extracted) {
    if (data->value.size != ARRAYLEN(tlv_extracted->output->not_valid_after)) {
        PRINTF("Invalid handle_not_valid_after format length %d\n", data->value.size);
        return false;
    }
    memcpy(tlv_extracted->output->not_valid_after,
           data->value.ptr,
           ARRAYLEN(tlv_extracted->output->not_valid_after));
    return true;
}
